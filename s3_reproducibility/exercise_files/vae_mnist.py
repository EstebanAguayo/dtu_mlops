...
def main(cfg):
    ...
    model = Model(Encoder=encoder, Decoder=decoder).to(torch.device("cuda" if cfg.hyperparameters.cuda else "cpu"))
    ...
    for epoch in range(cfg.hyperparameters.epochs):
        ...
        for batch_idx, (x, _) in enumerate(train_loader):
            ...
            x = x.to(torch.device("cuda" if cfg.hyperparameters.cuda else "cpu"))
    ...
    with torch.no_grad():
        for batch_idx, (x, _) in enumerate(test_loader):
            x = x.view(cfg.hyperparameters.batch_size, cfg.hyperparameters.x_dim)
            x = x.to("cuda" if cfg.hyperparameters.cuda else "cpu")      
            x_hat, _, _ = model(x)       
            break
    ...
    # Generate samples
    with torch.no_grad():
        noise = torch.randn(cfg.hyperparameters.batch_size, cfg.hyperparameters.latent_dim).to(torch.device("cuda" if cfg.hyperparameters.cuda else "cpu"))
